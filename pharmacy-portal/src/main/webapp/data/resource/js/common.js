...
function login_dialog() {
    CUR_DIALOG = ajax_form("login", "登录", SITEURL + "/index.php?c=login&inajax=1", 360, 1)
}
...
function collect_store(t, a, e) {
    $.get("index.php?c=index&op=login",
        function (i) {
            if (i == "0") {
                login_dialog()
            } else {
                var n = "index.php?c=member_favorites&op=favoritesstore";
                $.getJSON(n, {
                        fid: t
                    },
                    function (i) {
    ...
    $.get("index.php?c=index&op=login",
        function (i) {
            if (i == "0") {
                ...
                var n = "index.php?c=member_favorites&op=favoritesgoods";
                $.getJSON(n, {
                        fid: t
                    },
                    function (t) {
...
function load_cart_information() {
    $.getJSON(SITEURL + "/index.php?c=cart&op=ajax_load&callback=?",
        function (t) {
...
function drop_topcart_item(t) {
    $.getJSON(SITEURL + "/index.php?c=cart&op=del&cart_id=" + t + "&callback=?",
        function (a) {
            if (a.state) {
...
function load_history_information() {
    $.getJSON(SITEURL + "/index.php?c=index&op=viewed_info",
        function (t) {
...
(function (t) {
    t.show_nc_login = function (a) {
        var e = t.extend({},
            {
                cion: "/index.php?c=login&op=login&inajax=1",
                nchash: "",
                formhash: "",
                anchor: ""
            },
        ...
        i.append('<form class="bg" method="post" id="ajax_login" action="' + APP_SITE_URL + e.cion + '"></form>').find("form").append('<input type="hidden" value="ok" name="form_submit">').append('<input type="hidden" value="' + e.formhash + '" name="formhash">').append('<input type="hidden" value="' + e.nchash + '" name="nchash">').append('<dl><dt>用户名</dt><dd><input type="text" name="user_name" autocomplete="off" class="text"></dd></dl>').append('<dl><dt>密&nbsp;&nbsp;&nbsp;码</dt><dd><input type="password" autocomplete="off" name="password" class="text"></dd></dl>').append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'' + APP_SITE_URL + "/index.php?c=seccode&amp;op=makecode&amp;nchash=" + e.nchash + '&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="' + APP_SITE_URL + "/index.php?c=seccode&amp;op=makecode&amp;nchash=" + e.nchash + '" class="fl ml10"><span>不区分大小写</span></dd></dl>').append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="' + SHOP_SITE_URL + '/index.php?c=login&amp;op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="' + SHOP_SITE_URL + '/index.php?c=login&amp;op=forget_password">忘记密码</a>？，申请找回密码</li></ul>').append('<div class="enter"><input type="submit" name="Submit" value="&nbsp;" class="submit"></div><input type="hidden" name="ref_url" value="' + n + '">');
...
(function ($) {
    $.fn.membershipCard = function (options) {
        ...
        return this.each(function () {
            ...
            var _uri = SITEURL + "/index.php?c=member_card&callback=?&uid=" + data_str.id + "&from=" + options.type;
            var _dl = "";
            $this.qtip({
                content: {
                    text: "Loading...",
                    ajax: {
                        ...
                        success: function (data) {
                            if (data) {
                                ...
                                _dttmp.append('<i class="sex' + data.sex + '"></i>').append('<a href="' + SHOP_SITE_URL + "/index.php?c=member_snshome&mid=" + data.id + '" target="_blank">' + data.name + "</a>" + (data.truename != "" ? "(" + data.truename + ")" : ""));
                                if (options.type == "shop") {
                                    _dttmp.append(data.level_name ? '&nbsp;<div class="yks-grade-mini">' + data.level_name + "</div>" : "")
                                }
                                _dttmp.appendTo(_dl);
                                $('<dd class="avatar"><a href="' + SHOP_SITE_URL + "/index.php?c=member_snshome&mid=" + data.id + '" target="_blank"><img src="' + data.avatar + '" /></a><dd>').appendTo(_dl);
                                var _info = "";
                                ...
                                if (data.url != "") {
                                    $.getJSON(data.url + "/index.php?c=member_card&op=mcard_info&uid=" + data.id,
                                        function (d) {
                                            if (d) {
                                ...
                                if (data.follow != 2) {
                                    ...
                                    $('<div class="send-msg"> <a href="' + SHOP_SITE_URL + "/index.php?c=member_message&op=sendmsg&member_id=" + data.id + '" target="_blank"><i></i>站内信</a></div>').appendTo(_bottom)
                                }
        ...
        function follow(o) {
            ...
            eval("data_str = " + data_str);
            $.getJSON(SHOP_SITE_URL + "/index.php?c=member_snsfriend&op=addfollow&callback=?&mid=" + data_str.mid,
                function (t) {
                    if (t) {
        ...
        function onfollow(o) {
            var data_str = o.parent().attr("data-param");
            eval("data_str = " + data_str);
            $.getJSON(SHOP_SITE_URL + "/index.php?c=member_snsfriend&op=delfollow&callback=?&mid=" + data_str.mid,
                function (t) {
                    if (t) {
    ...
    t.fn.nc_region = function (a) {
        ...
        function l(a) {
            if (typeof nc_a === "undefined") {
                t.getJSON(SHOP_SITE_URL + "/index.php?c=index&op=json_area&callback=?",
                    function (t) {
                        nc_a = t;
                        a()
                    })
...
function addcart(goods_id, quantity, callbackfunc) {
    ...
    var url = "index.php?c=cart&op=add";
...
$(document).ready(function () {
    $('.am-tab-box li').click(function () {
        $(this).addClass('am-selected').siblings().removeClass('am-selected');
    })

    $('.ncc-receipt-infonew .ncc-table-style tr').each(function(){
        $(this).children('td:eq(-2)').css('color','#ff4400');               /* $(this).children('td').last().css('color','#ff4400');*/
    })
